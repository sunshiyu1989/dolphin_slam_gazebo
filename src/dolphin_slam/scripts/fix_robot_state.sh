#!/bin/bash
# Dolphin SLAM Robot State Node 修复脚本
# 解决 'x' 键错误问题

echo "🔧 修复 Robot State Node"
echo "========================="

WORKSPACE_DIR="$HOME/dolphin_slam_ws"
NODES_DIR="$WORKSPACE_DIR/src/dolphin_slam/nodes"
ROBOT_STATE_FILE="$NODES_DIR/robot_state_node.py"

# 检查文件是否存在
if [ ! -f "$ROBOT_STATE_FILE" ]; then
    echo "❌ 错误：找不到 robot_state_node.py"
    exit 1
fi

echo "📁 文件位置: $ROBOT_STATE_FILE"

# 备份原文件
BACKUP_FILE="$ROBOT_STATE_FILE.backup_$(date +%Y%m%d_%H%M%S)"
cp "$ROBOT_STATE_FILE" "$BACKUP_FILE"
echo "✅ 已备份到: $BACKUP_FILE"

# 应用修复
echo "🔧 应用修复..."

cat > "$ROBOT_STATE_FILE" << 'EOF'
#!/usr/bin/env python3
"""
...
if [ $? -eq 0 ]; then
    ...
    echo "   ./start_dolphin.sh"
    echo ""
    echo "🔍 预期结果："
    echo "   ✅ robot_state_node: 不应再有 'x' 键错误"
    echo "   ✅ 应该看到正常的坐标转换和数据加载"
    echo "   ✅ 节点应该持续运行而不过早退出"
else
    echo "❌ 构建失败，请检查错误信息"
    exit 1
...
echo ""
echo "📋 主要修复内容："
echo "  🔧 正确处理 latitude/longitude → x/y 坐标转换"
echo "  🔧 确保 current_pose 字典包含所有必需键"
echo "  🔧 添加数据验证和错误处理"
echo "  🔧 修复 ROS2 消息类型转换问题"
echo "  🔧 改进调试信息输出"
echo ""
echo "如果还有问题，请提供新的错误日志!"